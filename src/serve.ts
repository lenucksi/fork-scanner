...
function generateNavBar(outputDir: string, currentFile?: string): string {
  ...
  // Filter by report type if on a report page
  const currentStage = currentFile ? (currentFile.match(/-(?:stage|stufe)(\d+)/) || [])[1] : "";
  let options = "";
  for (const r of reports) {
    const fp = join(outputDir, r);
    const rStage = (r.match(/-(?:stage|stufe)(\d+)/) || [])[1];
    ...
    options += '<option value="/' + r + '"' + selected + ">" + (label || r.replace(/\.html$/, "").replace("report-", "").replace("stufe", "Stage ")) + "</option>";
  }
